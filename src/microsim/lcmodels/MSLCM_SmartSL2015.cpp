...
void
MSLCM_SmartSL2015::initDerivedParameters() {
...
bool
MSLCM_SmartSL2015::debugVehicle() const {
    return DEBUG_COND;
}


int
MSLCM_SmartSL2015::wantsChangeSublane(
        int laneOffset,
    ...
    if (myOwnState&LCA_BLOCKED && remainingTime()<=0) std::cerr << myVehicle.getID()  << std::endl;
    MSLeaderDistanceInfo* leaders2 = & (const_cast<MSLeaderDistanceInfo&>(leaders));
    MSLeaderDistanceInfo* followers2 = & (const_cast<MSLeaderDistanceInfo&>(followers));
    MSLeaderDistanceInfo* neighLeaders2 = & (const_cast<MSLeaderDistanceInfo&>(neighLeaders));
    MSLeaderDistanceInfo* neighFollowers2 = & (const_cast<MSLeaderDistanceInfo&>(neighFollowers));

    switch (myGroupState) {
        case LEADER:  leaders2 = smartLeaderDistance(leaders);
                      followers2 = smartFollowerDistance(followers);
                      neighLeaders2 = smartLeaderDistance(neighLeaders);
                      neighFollowers2 = smartFollowerDistance(neighFollowers);
            break;
        case MEMBER:  leaders2 = new MSLeaderDistanceInfo(leaders);
                      leaders2->clear();
                      neighLeaders2 = new MSLeaderDistanceInfo(neighLeaders);
                      neighLeaders2->clear();
            break;
    }
    ...
    int result;
    result = _wantsChangeSublane(laneOffset,
                                     alternatives,
                                     *leaders2, *followers2, blockers,
                                     *neighLeaders2, *neighFollowers2, neighBlockers,
                                     neighLane, preb,
                                     lastBlocked, firstBlocked, latDist, maneuverDist, blocked);

    result = keepLatGap(result, *leaders2, *followers2, blockers,
                        *neighLeaders2, *neighFollowers2, neighBlockers,
                        neighLane, laneOffset, latDist, maneuverDist, blocked);

    result |= getLCA(result, latDist);

    if (myGroupState == LEADER || myGroupState == MEMBER) {
        delete leaders2;
        delete neighLeaders2;
    }
    if (myGroupState == LEADER) {
        delete neighFollowers2;
        delete followers2;
    }
...
void
MSLCM_SmartSL2015::setOwnState(const int state) {
...
void
MSLCM_SmartSL2015::updateSafeLatDist(const double travelledLatDist) {
...
double
MSLCM_SmartSL2015::patchSpeed(const double min, const double wanted, const double max, const MSCFModel& cfModel) {
    gDebugFlag2 = DEBUG_COND;
...
double
MSLCM_SmartSL2015::_patchSpeed(const double min, const double wanted, const double max, const MSCFModel& cfModel) {
    if (wanted <= 0) {
        return wanted;
    }
...
void
MSLCM_SmartSL2015::msg(const CLeaderDist& cld, double speed, int state) {
    assert(cld.first != 0);
...
double
MSLCM_SmartSL2015::informLeader(int blocked,
                           int dir,
...
void
MSLCM_SmartSL2015::informFollower(int blocked,
                             int dir,
...
double
MSLCM_SmartSL2015::informLeaders(int blocked, int dir,
...
void
MSLCM_SmartSL2015::informFollowers(int blocked, int dir,
                              const std::vector<CLeaderDist>& blockers,
                              double remainingSeconds,
                              double plannedSpeed) {
    // #3727
...
int
MSLCM_SmartSL2015::computeSublaneShift(const MSEdge* prevEdge, const MSEdge* curEdge) {
...
int
MSLCM_SmartSL2015::_wantsChangeSublane(
        int laneOffset,
        LaneChangeAction alternatives,
        const MSLeaderDistanceInfo& leaders,
        const MSLeaderDistanceInfo& followers,
...
int
MSLCM_SmartSL2015::slowDownForBlocked(MSVehicle** blocked, int state) {
...
void
MSLCM_SmartSL2015::saveBlockerLength(const MSVehicle* blocker, int lcaCounter) {
...
void MSLCM_SmartSL2015::addLCSpeedAdvice(const double vSafe) {
...
void
MSLCM_SmartSL2015::updateExpectedSublaneSpeeds(const MSLeaderDistanceInfo& ahead, int sublaneOffset, int laneIndex) {
...
double
MSLCM_SmartSL2015::computeSpeedGain(double latDistSublane, double defaultNextSpeed) const {
...
CLeaderDist
MSLCM_SmartSL2015::getLongest(const MSLeaderDistanceInfo& ldi) {
    int iMax = 0;
    double maxLength = -1;
...
CLeaderDist
MSLCM_SmartSL2015::getSlowest(const MSLeaderDistanceInfo& ldi) {
    int iMax = 0;
...
int
MSLCM_SmartSL2015::checkBlocking(const MSLane& neighLane, double& latDist, double& maneuverDist, int laneOffset,
                            const MSLeaderDistanceInfo& leaders,
                            const MSLeaderDistanceInfo& followers,
                            const MSLeaderDistanceInfo& /*blockers */,
                            const MSLeaderDistanceInfo& neighLeaders,
    ...
    int blockedFully = 0;
    blockedFully |= checkBlockingVehicles(&myVehicle, leaders, myManeuverDist, myVehicle.getLane()->getRightSideOnEdge(), true, LCA_BLOCKED_BY_LEADER,
                                          mySafeLatDistRight, mySafeLatDistLeft, collectLeadBlockers);
    blockedFully |= checkBlockingVehicles(&myVehicle, followers, myManeuverDist, myVehicle.getLane()->getRightSideOnEdge(), false, LCA_BLOCKED_BY_FOLLOWER,
                                          mySafeLatDistRight, mySafeLatDistLeft, collectFollowBlockers);
    if (laneOffset != 0) {
        blockedFully |= checkBlockingVehicles(&myVehicle, neighLeaders, myManeuverDist, neighLane.getRightSideOnEdge(), true,
                                              (laneOffset == -1 ? LCA_BLOCKED_BY_RIGHT_LEADER : LCA_BLOCKED_BY_LEFT_LEADER),
                                              mySafeLatDistRight, mySafeLatDistLeft, collectLeadBlockers);
        blockedFully |= checkBlockingVehicles(&myVehicle, neighFollowers, myManeuverDist, neighLane.getRightSideOnEdge(), false,
                                              (laneOffset == -1 ? LCA_BLOCKED_BY_RIGHT_FOLLOWER : LCA_BLOCKED_BY_LEFT_FOLLOWER),
                                              mySafeLatDistRight, mySafeLatDistLeft, collectFollowBlockers);
    }
...
int
MSLCM_SmartSL2015::checkBlockingVehicles(
...
void
MSLCM_SmartSL2015::updateCFRelated(const MSLeaderDistanceInfo& vehicles, double foeOffset, bool leaders) {
...
bool
MSLCM_SmartSL2015::overlap(double right, double left, double right2, double left2) {
    assert(right <= left);
    assert(right2 <= left2);
...
MSLCM_SmartSL2015::StateAndDist
MSLCM_SmartSL2015::decideDirection(StateAndDist sd1, StateAndDist sd2) const {
...
LaneChangeAction
MSLCM_SmartSL2015::getLCA(int state, double latDist) {
...
int
MSLCM_SmartSL2015::checkStrategicChange(int ret,
                                   int laneOffset,
                                   const std::vector<MSVehicle::LaneQ>& preb,
                                   const MSLeaderDistanceInfo& leaders,
                                   const MSLeaderDistanceInfo& neighLeaders,
...
double
MSLCM_SmartSL2015::computeGapFactor(int state) const {
    return (state & LCA_STRATEGIC) != 0 ? MAX2(0.0, (1.0 - myPushy * (1 + 0.5 * myImpatience))) : 1.0;
}


int
MSLCM_SmartSL2015::keepLatGap(int state,
                         const MSLeaderDistanceInfo& leaders,
                         const MSLeaderDistanceInfo& followers,
                         const MSLeaderDistanceInfo& blockers,
                         const MSLeaderDistanceInfo& neighLeaders,
...
void
MSLCM_SmartSL2015::updateGaps(const MSLeaderDistanceInfo& others, double foeOffset, double oldCenter, double gapFactor,
...
double
MSLCM_SmartSL2015::getWidth() const {
    return myVehicle.getVehicleType().getWidth() + NUMERICAL_EPS;
}


double
MSLCM_SmartSL2015::computeSpeedLat(double latDist, double& maneuverDist) {
...
void
MSLCM_SmartSL2015::commitManoeuvre(int blocked, int blockedFully,
                              const MSLeaderDistanceInfo& leaders,
                              const MSLeaderDistanceInfo& neighLeaders,
                              const MSLane& neighLane,
                              double maneuverDist) {
...
double
MSLCM_SmartSL2015::commitFollowSpeed(double speed, double latDist, double secondsToLeaveLane, const MSLeaderDistanceInfo& leaders, double foeOffset) const {
...
double
MSLCM_SmartSL2015::getSafetyFactor() const {
    return 1 / ((1 + 0.5 * myImpatience) * myAssertive);
}


std::string
MSLCM_SmartSL2015::getParameter(const std::string& key) const {
...
void
MSLCM_SmartSL2015::setParameter(const std::string& key, const std::string& value) {
    double doubleValue;
    try {
...
MSLeaderDistanceInfo * MSLCM_SmartSL2015::smartLeaderDistance(const MSLeaderDistanceInfo &leaderDistanceInfo) {
    ...
    if (leaderDistanceInfo.hasVehicles()) {
        for (int i = 0; i < leaderDistanceInfo.numSublanes(); ++i) {
            if (leaderDistanceInfo[i].first != nullptr &&
                leaderDistanceInfo[i].first->getVehicleType().getLaneChangeModel() == LCM_SmartSL2015) {
                ...
                if (inFrontSAL != 0) {
                    ...
                    if (groupLeader == nullptr) copy->addLeader(leaderDistanceInfo[i].first, leaderDistanceInfo[i].second, 0, i);
                        ...
                        copy->addLeader((MSVehicle *) groupLeader,
                                        sqrt(pow(myPos.x()-frontPos.x(),2)+pow(myPos.y()-myPos.y(),2)) - 6.65, 0, i);
                    }
                }
            } else copy->addLeader(leaderDistanceInfo[i].first, leaderDistanceInfo[i].second, 0, i);
        }
    }
    if (DEBUG_COND)  std::cout << copy->toString() << std::endl << std::endl;
    return copy;
}

MSLeaderDistanceInfo * MSLCM_SmartSL2015::smartFollowerDistance(const MSLeaderDistanceInfo &followerDistanceInfo) {
    ...
    for (int i=0; i<followerDistanceInfo.numSublanes(); ++i){


        if (followerDistanceInfo[i].first != nullptr) {
            //if outside of grouping system, then do not bother him:
            MSLCM_SmartSL2015* follower = (MSLCM_SmartSL2015*) &const_cast<MSAbstractLaneChangeModel&>((followerDistanceInfo[i].first)->getLaneChangeModel());
            if (follower->myGroupState != MEMBER || follower->myGroupState != LEADER) {
                copy->addLeader(followerDistanceInfo[i].first, followerDistanceInfo[i].second, 0, i);
                continue;
            }

            if (followerDistanceInfo[i].second < 0) {
                SUMOVehicle *behind = (SUMOVehicle *) (const_cast<MSVehicle *>(followerDistanceInfo[i].first));
                ...
                if (behindSAL != 0) {
                    ...
                    if (groupLeader == followerDistanceInfo[i].first) {
                        copy->addLeader(followerDistanceInfo[i].first, followerDistanceInfo[i].second, 0, i);
                        continue;
                    }
                } else;
                    ...
                    if (follower->myVehicle.getPositionOnLane() < myVehicle.getPositionOnLane() || myVehicle.getLane() != (follower->myVehicle).getLane()) {
                        ...
                        if (behindSAL != 0) {
                            ...
                            if (groupLeader == (SUMOVehicle*)&(follower->myVehicle)) {
                                ...
                                copy->addLeader((MSVehicle*)const_cast<SUMOVehicle*>(groupLeader),
                            ...
                            copy->addLeader(followerDistanceInfo[i].first, followerDistanceInfo[i].second, 0,
                                                           i);
                    }
                ...
                if (!ok) copy->addLeader(followerDistanceInfo[i].first, followerDistanceInfo[i].second, 0,
                                                        i);
            }
        } else copy->addLeader(followerDistanceInfo[i].first, followerDistanceInfo[i].second, 0, i);
    }
    if (DEBUG_COND) std::cout << copy->toString() << std::endl << std::endl;
